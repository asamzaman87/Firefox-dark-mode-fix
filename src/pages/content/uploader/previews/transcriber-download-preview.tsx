...
const TranscriberDownloadPreview: FC<DownloadPreviewProps> = ({
  ...
  const isDownloadDisabled = cleanedText.trim().length === 0;
  ...
  return (
    <div className="gpt:flex gpt:flex-col gpt:items-center gpt:justify-center gpt:size-full gpt:gap-4">
      <div className="gpt:text-center">
          ...
          <h1 className="gpt:text-xl gpt:font-bold">
            {progress >= 0 &&
              ...
              `${chrome.i18n.getMessage(
                "loading_transcribe_text"
              )} (${progress.toFixed(0)}%)`}
            {progress === 100 &&
              `${chrome.i18n.getMessage(
                "transcribed_complete"
              )} (${progress.toFixed(0)}%)`}
          </h1>
        )}
        ...
        {hasError && (
          <h1 className="gpt:text-xl gpt:font-bold gpt:text-red-600">
            {`${chrome.i18n.getMessage("download_aborted")} (${progress.toFixed(
              0
            )}%)`}
          </h1>
        )}
        ...
        {!hasError && (
          <p className="gpt:text-sm gpt:text-gray-700 gpt:dark:text-gray-400">
            {progress === 100
              ? `${chrome.i18n.getMessage("full_download_note_t")}`
        ...
        {hasError && (
          <p className="gpt:text-red-500 gpt:text-wrap gpt:max-w-lg gpt:text-center">
            {progress === 0 && chrome.i18n.getMessage("error_no_start")}
            {progress > 0 && chrome.i18n.getMessage("error_stopped_midway")}
          </p>
        )}
      </div>
      ...
      <div className="gpt:relative gpt:size-full gpt:overflow-y-auto">
        ...
        >
          <Loader2 className="gpt:animate-spin" />{" "}
          {chrome.i18n.getMessage("loading_preview")}
        </span>
        <span
          className={cn(
              ...
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    handleDownload(text, format, isDownloadDisabled);
                    toast({
                      id: "recording-toast",
                      description:
                ...
                >
                  <DownloadCloud strokeWidth={2} className="size-5" />{" "}
