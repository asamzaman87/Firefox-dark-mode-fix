import { CHUNK_TO_PAUSE_ON, FORWARD_REWIND_TIME, LISTENERS, LOADING_TIMEOUT, LOADING_TIMEOUT_FOR_DOWNLOAD, PLAY_RATE_STEP, TOAST_STYLE_CONFIG } from "@/lib/constants";
...
const useAudioPlayer = (isDownload: boolean) => {
    ...
    const { blobs, downloadPreviewText, downloadCombinedFile, progress, setProgress, isFetching, wasPromptStopped, setWasPromptStopped, chunks, setIsPromptingPaused, isPromptingPaused, audioUrls, setAudioUrls, ended, extractText, splitAndSendPrompt, text, reset: resetAudioUrl, voices, setVoices, isVoiceLoading, is9ThChunk, reStartChunkProcess, setIs9thChunk, isLoading } = useAudioUrl(isDownload);
    ...
    const [prevCurrentPlayTime, setPrevCurrentPlayTime] = useState<number>(0);
    ...
    useMemo(() => {
        if (blobs.length > 0) {
            const combinedBlob = new Blob(blobs, {
              ...
              const audio = new Audio(combinedUrl);
              audio.addEventListener("loadedmetadata", () => {
                setPlayTimeDuration(audio.duration);
              });
        }
    },  [blobs]);
    ...
    audioPlayer.ontimeupdate = () => {
        const currentTimeDiff = audioPlayer.currentTime - prevCurrentPlayTime;
        setPrevCurrentPlayTime(audioPlayer.currentTime);
        setCurrentPlayTime(p => currentTimeDiff + p);
    }
    ...
    const onForward = useCallback(() => {
        ...
        // If the increased time exceeds the total duration, adjust the current time accordingly.
        // The `handleAudioEnd` function will play the next chunk if the current time surpasses the total duration or if the audio ends.
        // 
        // To ensure the new chunk starts at the correct position, we increase the time by `(increasedTime - totalDuration)`.
        // Example:
        // - Current time: 10 seconds
        // - Total duration: 20 seconds
        // - Increased time: 30 seconds
        //
        // Since 30 seconds is 10 seconds beyond the total duration, we set the time to:
        // `(increasedTime - totalDuration)` (which equals 10 seconds in this case).
        //
        // This ensures the new audio chunk starts at the correct position (10 seconds).
        if (increasedTime > totalDuration) {
            const newTime = increasedTime - totalDuration;
            audioPlayer.currentTime = newTime;
            return;
        }
        audioPlayer.currentTime = increasedTime;
    }, [audioPlayer])
    ...
    const onRewind = useCallback(() => {
        ...
        // If the reduced time is less than or equal to zero, adjust the current time accordingly to play the previous chunk.
        //
        // To ensure the previous chunk starts at the correct position, we calculate the starting time as follows:
        // `totalDurationOfPreviousChunk - Math.abs(reducedTimeFromPrevious)`
        //
        // Example:
        // - Current time: 6 seconds
        // - Reduced time: `currentTime - 10 = -4`; where 10 is the rewind amount
        //
        // Since the reduced time (-4) is less than zero, we need to play the previous chunk.
        // Assuming the total duration of the previous chunk is 20 seconds:
        // - `Math.abs(reducedTimeFromPrevious) = 4`
        // - The previous chunk will start playing from `20 - 4 = 16` seconds.
        //
        // This ensures smooth playback, resuming from the correct position within the previous chunk.        
        if (reducedTime <= 0 && currentIndex > 0 && currentIndex < chunks.length - 1) {
            playNext(currentIndex - 1); //play previous chunk
            setCurrentIndex(c => c - 1); //decrease current index

            //playing after a delay to allow the audio to set as we are using refs and we cannot detect ref state change
            return setTimeout(() => {
                const totalDuration = audioPlayer.duration;
                const newTime = totalDuration - Math.abs(reducedTime);
                audioPlayer.currentTime = newTime;
            }, 200)
        }

        audioPlayer.currentTime = reducedTime;
